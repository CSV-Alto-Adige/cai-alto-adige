...
export default function FilterModal({
  ...
  const uniqueOrganizerSections: OrganizerSection[] = filterOptions.sezioni.map(
    (name, index) => ({
      id: index + 1,
      name,
      avatar:
  ...
  const targetGroup: TargetGroup[] = filterOptions.gruppi.map(
    (name, index) => ({
      id: index + 1,
      name,
    })
  ...
  const difficulty: DifficultyLevel[] = filterOptions.difficolta.map(
    (name, index) => ({
      id: index + 1,
      name,
    })
