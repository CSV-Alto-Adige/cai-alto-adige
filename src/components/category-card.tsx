...
export default function CategoryCard({ onCategorySelect, activityType, count, selectedCategory }: CategoryCardProps) {
  ...
  const [isDisabled, setIsDisabled] = useState(selectedCategory === activityType.name);
  ...
  useEffect(() => {
    if (shouldReset) {
      ...
      setIsDisabled(selectedCategory === activityType.name);
    }
  }, [shouldReset, selectedCategory, activityType.name]);
